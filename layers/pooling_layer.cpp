    ...
    switch (this->layer_param_.pooling_param().pool()){
        case PoolingParameter_PoolMethod_MAX:
            if(use_top_mask){
                ...
                caffe_set(top_count, Dtype(-1), top_mask);     // 为top_mask申请内存，并用常数alpha对Y进行初始化
            }else{
                mask = top[0]->mutable_cpu_data();
                caffe_set(top_count, -1, mask);
            }
            caffe_set(top_count, Dtype(-FLT_MAX), top_data);    // 对top_data进行初始化
